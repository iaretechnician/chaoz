    ...
    public class PlayerEquipment : Equipment
    {
        ...
        public void SwapInventoryEquip(int inventoryIndex, int equipmentIndex)
        {
                ...
                0 <= equipmentIndex && equipmentIndex < slots.Count)
            {
                ...
                if (slot.amount == 0 ||
                    slot.item.data is EquipmentItem item &&
                    item.CanEquip(this, inventoryIndex, equipmentIndex))
                {
                    // swap them
        ...
        public void OnRespawn()
        {
            ...
            for (int i = 0; i < slotInfo.Length; ++i)
                slots[i] = slotInfo[i].defaultItem.item != null
                    ? new ItemSlot(new Item(slotInfo[i].defaultItem.item), slotInfo[i].defaultItem.amount)
                    : new ItemSlot();
        }
